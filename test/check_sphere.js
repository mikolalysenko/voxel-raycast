...
test("sphere", function(t) {
  ...
  for(var theta=0; theta<Math.PI * 2.0; theta+=0.25) {
    for(var phi=0; phi<Math.PI; phi+=0.25) {
      var dir = [ Math.cos(theta) * Math.cos(phi), Math.sin(theta) * Math.cos(phi), Math.sin(phi) ]    
      for(var x=0.0; x<1.0; x+=0.25) {
        for(var y=0.0; y<1.0; y+=0.25) {
          for(var z=0.0; z<1.0; z+=0.25) {
